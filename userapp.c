...
int main(int argc, char* argv[]){
    ...
    while(t < num){
        gettimeofday(&start, NULL);
        printf("pid is %u, job %d is running, start time:\t%llu.%llu s\n", pid, t, (unsigned long long)start.tv_sec, (unsigned long long)start.tv_usec);
        do_job(process_time);
        gettimeofday(&end, NULL);
        printf("pid is %u, job %d finished, end time:    \t%llu.%llu s\n", pid, t, (unsigned long long)end.tv_sec, (unsigned long long)end.tv_usec);
        YIELD(pid);
        t++;
    }
